//! AD3038: EnableUBSan
//!
//! Checks that UndefinedBehaviorSanitizer (UBSan) is enabled in ELF binaries.
...
pub struct EnableUBSan {
    descriptor: RuleDescriptor,
}

impl EnableUBSan {
    pub fn new() -> Self {
        let descriptor = RuleDescriptor::new(AD3038, "EnableUBSan")
            .with_category(RuleCategory::Security)
            .with_tags(&["debug-only"])
...
impl Default for EnableUBSan {
    fn default() -> Self {
        Self::new()
    }
}

impl Rule for EnableUBSan {
    ...
    fn analyze(&self, context: &mut AnalysisContext) {
        ...
        if elf.has_any_symbol(UBSAN_SYMBOLS) {
