...
//!
//! Checks that ELF binaries do not use known dangerous/banned API functions.
...
impl DoNotUseBannedApisELF {
    pub fn new() -> Self {
        let descriptor = RuleDescriptor::new(AD3041, "DoNotUseBannedApisELF")
            .with_category(RuleCategory::Security)
            .with_tags(&["hardening", "memory-safety"])
            ...
            .with_full_description(
                "Certain C library functions are known security risks due to lack of bounds \
                 ...
                 strncpy/strlcpy, snprintf, and fgets. See also the OpenSSF Compiler Hardening Guide.",
            )
            .with_fix_hint("Replace banned functions with safer alternatives (e.g., strncpy, snprintf, fgets)")
...
impl Default for DoNotUseBannedApisELF {
    fn default() -> Self {
        Self::new()
    }
}

impl Rule for DoNotUseBannedApisELF {
    ...
    fn analyze(&self, context: &mut AnalysisContext) {
        ...
        let critical_found: Vec<&str> = CRITICAL_BANNED_APIS
            .iter()
            .filter(|api| elf.has_symbol_exact(api))
            .copied()
            .collect();
