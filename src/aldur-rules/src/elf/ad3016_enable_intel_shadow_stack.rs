...
//!
//! Checks that x86-64 binaries have Intel Shadow Stack (SHSTK) enabled
//! as part of Control-flow Enforcement Technology (CET).
...
use crate::rule_ids::AD3016;

pub struct EnableIntelShadowStack {
    descriptor: RuleDescriptor,
}

impl EnableIntelShadowStack {
    pub fn new() -> Self {
        let descriptor = RuleDescriptor::new(AD3016, "EnableIntelShadowStack")
            .with_category(RuleCategory::Security)
            .with_tags(&["recommended", "control-flow", "intel-only", "rhel-annocheck", "openssf"])
            .with_short_description("Enable Intel Shadow Stack (CET-SS).")
            .with_full_description(
                "Intel Shadow Stack provides hardware-based protection against Return-Oriented \
                 Programming (ROP) attacks by maintaining a shadow copy of return addresses. \
                 If the return address on the stack doesn't match the shadow stack, execution \
                 is halted. Compile with '-fcf-protection=full' to enable Shadow Stack support.",
            )
            .with_default_level(FailureLevel::Warning)
            .with_message(
                "Pass",
                "'{0}' has Intel Shadow Stack enabled.",
            )
            .with_message(
                "Warning",
                "'{0}' does not have Intel Shadow Stack enabled. Compile with \
                 '-fcf-protection=full' to enable hardware-based ROP protection.",
            )
            .with_message(
                "NotApplicable_NotX86_64",
                "'{0}' is not an x86-64 binary. Intel Shadow Stack only applies to x86-64.",
            )
            .with_message(
            ...
            .with_fix_hint("Compile with -fcf-protection=full and link with -Wl,-z,shstk.");

        Self { descriptor }
    }
}

impl Default for EnableIntelShadowStack {
    fn default() -> Self {
        Self::new()
    }
}

impl Rule for EnableIntelShadowStack {
